...
export default
function NotesTab({ course, slug, isAdmin, onReloadCourse, initialSectionId }: { course: any; slug: string; isAdmin?: boolean; onReloadCourse?: () => void; initialSectionId?: string }) {
...
<body>
  ...
  return (
    <section className="space-y-6" aria-label="Ders notları">
                  ...
                  <div className="mb-4 p-3 rounded-xl bg-white/[0.02] border border-white/[0.06]">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {sectionHighlights[section.id]?.map(hl => (
                        <span
                          key={hl.id}
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] border ${getColorClass(hl.color)}`}
                        >
      ...
      {highlightPopup && (
        <div
          className="highlight-popup fixed z-50 flex items-center gap-1 px-2 py-1.5 rounded-xl bg-slate-800 border border-white/10 shadow-2xl shadow-black/40"
          style={{
            left: `${Math.min(highlightPopup.x, window.innerWidth - 180)}px`,
            top: `${highlightPopup.y - 40}px`,
          }}
        >
          <span className="text-[10px] text-slate-400 mr-1">İşaretle:</span>
          ...
          <button
            ...
            className="ml-1 p-1 rounded-lg hover:bg-white/10 text-slate-400"
          >
            <X className="w-3.5 h-3.5" />
          </button>
        </div>
