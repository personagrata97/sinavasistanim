...
import { HelpCircle, CheckCircle2, XCircle, ChevronRight, AlertTriangle, Flag, Lightbulb, RefreshCw, Brain, CheckCircle, Target, Zap, Loader2, Download, Sparkles, Star, BookOpen } from "lucide-react"
import ReactMarkdown from "react-markdown"
import remarkGfm from "remark-gfm"
import rehypeRaw from "rehype-raw"
import { toast } from "sonner"
import { EmptyState, LoadingSkeleton, Badge, ConfettiEffect, formatTitle, Modal, cleanExplanationText, SplitNotesLayout } from "./shared"
      ...
      {!quickReview && (
        <div className="space-y-3">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex-1 bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 flex items-center gap-3">
              <label htmlFor="q-topic-filter" className="text-xs font-bold text-slate-500 whitespace-nowrap">Konu:</label>
              <select 
                id="q-topic-filter"
                aria-label="Konu filtresi"
                value={topicFilter}
                onChange={(e) => { setTopicFilter(e.target.value); setCurrentQ(0); }}
                className="bg-transparent text-sm font-medium w-full focus:outline-none text-slate-300"
              >
                <option value="all" className="bg-[#0f172a]">Tümü</option>
                {allTopics.map(t => <option key={t} value={t} className="bg-[#0f172a]">{t}</option>)}
              </select>
            </div>
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 flex items-center gap-3">
              <label htmlFor="q-status-filter" className="text-xs font-bold text-slate-500 whitespace-nowrap">Durum:</label>
              <select
                id="q-status-filter"
                aria-label="Durum filtresi"
                value={statusFilter}
                onChange={(e) => { setStatusFilter(e.target.value); setCurrentQ(0); }}
                className="bg-transparent text-sm font-medium focus:outline-none text-slate-300"
              >
                <option value="all" className="bg-[#0f172a]">Tümü</option>
                <option value="unsolved" className="bg-[#0f172a]">Çözülmemiş</option>
                <option value="wrong" className="bg-[#0f172a]">Yanlışlarım</option>
                <option value="flagged" className="bg-[#0f172a]">Yıldızladıklarım</option>
              </select>
            </div>
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-2.5 flex items-center gap-3">
              <label htmlFor="q-diff-filter" className="text-xs font-bold text-slate-500 whitespace-nowrap">Zorluk:</label>
              <select
                id="q-diff-filter"
                aria-label="Zorluk filtresi"
                value={difficultyFilter}
                onChange={(e) => { setDifficultyFilter(e.target.value); setCurrentQ(0); }}
                className="bg-transparent text-sm font-medium focus:outline-none text-slate-300"
              >
                <option value="all" className="bg-[#0f172a]">Hepsi</option>
                <option value="easy" className="bg-[#0f172a]">Kolay</option>
                <option value="medium" className="bg-[#0f172a]">Orta</option>
                <option value="hard" className="bg-[#0f172a]">Zor</option>
              </select>
            </div>
          </div>
          {/* Butonlar — ayrı satır */}
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 pt-8 border-t border-white/5">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <Tooltip content={flagged.has(q.id) ? "Bu soru bildirildi" : "Bu soruyu hatalı olarak bildir"}>
                  <button
                    onClick={async () => {
                      const { reportQuestion } = await import("@/lib/actions")
                      await reportQuestion(q.id)
                      setFlagged(prev => new Set([...prev, q.id]))
                      toast.success("Soru hatalı olarak bildirildi. Teşekkürler!")
                    }}
