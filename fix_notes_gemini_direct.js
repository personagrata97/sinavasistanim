...
6. Hiçbir şekilde "Üzgünüm", "Eksik metin", "Hata" gibi şeyler yazma. Ne verilmişse onu özetle.`;
...
db.serialize(() => {
  db.all(`SELECT id, rawContent FROM Section WHERE id IN (?, ?, ?)`, sectionIds, async (err, rows) => {
    ...
    for (const row of rows) {
      console.log(`Generating for ${row.id}... (Raw length: ${row.rawContent.length})`);
