...
async function run() {
  console.log("🚀 [MASTER QUALITY ENGINE] Starting full 20-section auto-refinement for Bilgi Sistemleri Güvenliği...")
  ...
  // Fetch all 20 sections
  const sections = await prisma.section.findMany({
    where: { courseId: course.id },
    orderBy: { order: "asc" }
  })

  console.log(`Found ${sections.length} sections in Bilgi Güvenliği course to process.`)
  ...
  for (const sec of sections) {
    const qCount = await prisma.question.count({ where: { sectionId: sec.id } })
    const fCount = await prisma.flashcard.count({ where: { sectionId: sec.id } })

    if (sec.verificationScore === 100 && sec.notes && sec.notes.length > 100 && qCount >= 10 && fCount >= 10) {
      console.log(`🔷 [BÖLÜM ${sec.order}/${sections.length}] "${sec.title}" is already at 100% with ${qCount} questions and ${fCount} flashcards. Skipping!`)
      continue
    }
    console.log(`\n============================================================`)
    ...
    if (!sec.notes || sec.notes.length < 100 || currentScore === 0) {
      ...
      const aiMode = course.program?.aiMode || "finance"
    ...
    // Nihai Müfettiş sonucunu kaydet
    ...
    if (finalSec) {
      ...
      const finalScore = finalSec.verificationScore ?? currentScore
      await prisma.section.update({
        where: { id: sec.id },
        data: {
          verificationScore: finalScore,
          verificationIssues: JSON.stringify(issuesObj),
          processed: finalScore >= 95
        }
      })
    }
  ...
  console.log("\n============================================================")
  console.log("🎉🎉🎉 [BİLGİ GÜVENLİĞİ DERSİNİN TAMAMI KUSURSUZLAŞTIRILDI] Master Quality Engine başarıyla tamamlandı!")
}
